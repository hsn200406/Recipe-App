...
function TabIcon({ icon, label, focused }) {
  ...
  return (
    <View style={{ alignItems: 'center', gap: 2, width: 80, paddingTop: 10 }}>
      ...
      <Text style={{ fontSize: 10, color: focused ? theme.accent : theme.muted, fontWeight: focused ? '600' : '400' }}>
        {label}
      </Text>
    </View>
  );
  ...
  return (
    ...
    >
      ...
      <Tab.Screen name="Saved"   component={SavedScreen}
        options={{ tabBarIcon: ({ focused }) => <TabIcon icon="🔖" label="Saved"    focused={focused} /> }} />
