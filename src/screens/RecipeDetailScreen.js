                ...
                {[1, 2, 3, 4, 5].map((s) => (
                  <TouchableOpacity key={s} onPress={() => setStars(s)}>
                    <Text
                      style={{
                        fontSize: 38,
                        color: s <= stars ? theme.gold : theme.border,
                      }}
                    >
                      ★
                    </Text>
...
export default function RecipeDetailScreen() {
  ...
  const toggleStep = (i) =>
    setChecked((cs) =>
      cs.includes(i) ? cs.filter((x) => x !== i) : [...cs, i],
    );
    ...
    <SafeAreaView style={[s.safe, { backgroundColor: theme.bg }]}>
      ...
      >
        ...
        >
          ...
          />
          <View style={{ flex: 1 }}>
            <Text style={[s.creatorName, { color: theme.text }]}>
              {creator.name}
            </Text>
          ...
          {[
            ...
            <TouchableOpacity
              ...
              style={[
                s.actionBtn,
                {
                  backgroundColor: btn.active ? btn.color + "18" : theme.card,
                  borderColor: btn.active ? btn.color : theme.border,
                },
              ]}
            >
              <Text
        ...
        {tab === "ingredients" && (
          <View>
            {safeRecipe.ingredients.map((ing, i) => (
              <View
                key={i}
                style={[s.ingRow, { borderBottomColor: theme.border }]}
              >
                <Text style={[s.ingQty, { color: recipe.accentColor }]}>
                  {ing.qty}
                </Text>
                <Text style={[s.ingName, { color: theme.text }]}>
                  {ing.name}
                </Text>
              </View>
            ))}
            ...
            {reviews.map((r) => (
              ...
              >
                <View style={s.reviewHeader}>
                  <Avatar
                    initial={
                      r.initial || r.user?.charAt(0)?.toUpperCase() || "?"
                    }
                    color={r.color || theme.accent}
                    size={28}
                  />
                  <Text style={[s.reviewUser, { color: theme.text }]}>
                    @{r.user}
                  </Text>
                  {r.rating > 0 && <Stars rating={r.rating} size={12} />}
                  <Text style={[s.reviewTime, { color: theme.muted }]}>
                    {r.time}
                  </Text>
                </View>
                <Text style={[s.reviewText, { color: theme.muted }]}>
                  {r.text}
                </Text>
              </View>
            ))}
