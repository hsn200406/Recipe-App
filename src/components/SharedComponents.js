...
export function MacroBar({ label, value, max, color }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <Text style={{ fontSize: 10, color: theme.muted, marginBottom: 3 }}>{label}</Text>
      <View style={{ backgroundColor: theme.border, borderRadius: 4, height: 5, overflow: 'hidden' }}>
        <View style={{ width: `${pct}%`, backgroundColor: color, height: '100%', borderRadius: 4 }} />
      </View>
      <Text style={{ fontSize: 11, color: theme.text, marginTop: 3, fontWeight: '500' }}>{value}g</Text>
    </View>
  );
}
...
export function ScreenHeader({ title, onBack, right }) {
  ...
  return (
    <View style={{
      ...
      <Text style={{ flex: 1, fontSize: 18, fontWeight: '700', color: theme.text }}>{title}</Text>
      {right}
    </View>
  );
}
